...
def compareloss(input,temperature):
    ...
    temperature=torch.tensor(temperature,dtype=float).cuda()
    a=torch.index_select(input,dim = 1,index = torch.tensor([0]).cuda()).reshape(input.shape[0],input.shape[2]).cuda()
    positive=torch.index_select(input,dim = 1,index = torch.tensor([1]).cuda()).reshape(input.shape[0],input.shape[2]).cuda()
    negative=torch.index_select(input,dim = 1,index = torch.tensor([2]).cuda()).reshape(input.shape[0],input.shape[2]).cuda()
...
def distance2center(input,center):
    ...
    m = input.size(1)
    k = center.size(0)
    input_power = torch.sum(input * input, dim=1, keepdim=True).expand(n, k)  # 对应元素相乘
    ...
    distance = input_power + center_power - 2 * torch.mm(input, center.transpose(0, 1))  # 矩阵的乘法
    return distance
...
def center_embedding(embedding, node_label, label_num, debug=False):
    """
    向量化版本的中心嵌入计算
    """
    ...
    # 创建 one-hot 编码
    one_hot = torch.zeros(node_label.size(0), label_num).to(device)
    one_hot.scatter_(1, node_label.unsqueeze(1), 1)

    # 计算每个类别的样本数量
    counts = one_hot.sum(dim=0, keepdim=True)  # [1, label_num]
    counts = counts.clamp(min=1)  # 避免除以0

    # 计算中心嵌入：one_hot^T @ embedding / counts^T
    center_emb = torch.mm(one_hot.t(), embedding) / counts.t()  # [label_num, feat_dim]
...
def index2mask(index,nodenum):
    ...
    ret = torch.zeros(nodenum, dtype=torch.bool)
    ones = torch.ones(index.shape[0], dtype=torch.bool)
