...
from AMSS import  amss_complete
...
train_config = {
    ...
    ################################################################################
    "save_model_dir": "./save_model/luo_model/10000_max_test/",
    #"save_model_dir": "./save_model/zheng_model/10000_max_test/",
    ############################################################################
...
def train(model, optimizer, scheduler, data_type, data_loader, device, config, epoch, logger=None, writer=None):
        ...
        if config["amss_enabled"]:
            ...
            for name, param in model.named_parameters():
                if param.requires_grad and param.grad is not None:
                    ...
                    if config["amss_importance_type"] == "squared_grad":
                        current_importance = param.grad.data.pow(2)
                    elif config["amss_importance_type"] == "abs_grad":
                        current_importance = param.grad.data.abs()
                    else:
                        raise ValueError(f"Unknown amss_importance_type: {config['amss_importance_type']}")


                    if name not in param_importances:
                        param_importances[name] = torch.zeros_like(param.data, device=device)
                    param_importances[name].mul_(config["amss_importance_momentum"]).add_(
                        current_importance, alpha=(1 - config["amss_importance_momentum"])
                    )


                    num_elements = param.numel()
                    num_to_update = int(num_elements * config["amss_update_ratio"])
            ...
            if logger and writer and batch_id % config["print_every"] == 0:
                writer.add_scalar("AMSS/Total_Params_Masked_Batch", total_params_masked,
                                  epoch * epoch_step + batch_id)
    ...
    if config["amss_enabled"] and writer:
        total_grad_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        total_importance_sum = sum(pi.sum().item() for pi in param_importances.values())
...
def evaluate(model, data_type, data_loader, device, config, epoch, logger=None, writer=None):
    ...
    with torch.no_grad():
        for batch_id, batch in enumerate(data_loader):
            ...
            pred = torch.gather(input=pred, dim=0, index=sample)
...
if __name__ == "__main__":
    ...
    ##########################################################################
    save_path_train = "../get_nodefeature_module/graph_dti/zheng_graph_dti/n(6,3)_all_link_random/graph_train"
    save_path_dev = "../get_nodefeature_module/graph_dti/zheng_graph_dti/n(6,3)_all_link_random/graph_dev"
    save_path_all = "../get_nodefeature_module/graph_dti/zheng_graph_dti/n(6,3)_all_link_random/graph_all"
    ############################################################################
    id_num = 0
    label_num = 0
